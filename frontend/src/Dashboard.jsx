...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Update current date in German format (e.g., 12. APR. 2026)
    const now = new Date();
    ...
    // Fetch real transactions from your Render API
    axios
  ...
  return (
    <div className="max-w-sm mx-auto h-screen flex flex-col bg-white font-sans overflow-hidden">
      ...
      {/* TOP NAV ICONS */}
      <div className="absolute top-6 right-6 flex gap-6 z-10 text-white">
      ...
      <div className="bg-gradient-to-b from-[#001E62] to-[#003DA5] pt-16 pb-12 px-6 rounded-b-[40px] text-center text-white relative overflow-hidden">
        ...
        <div className="flex justify-around gap-4 px-4 relative z-10">
          ...
          <ActionButton icon={<div className="w-6 h-5 border-2 border-white flex items-center justify-around px-0.5">
            <div className="w-px h-3 bg-white" /><div className="w-px h-3 bg-white" /><div className="w-px h-3 bg-white" />
          </div>} label="Bargeld-Code" />
      ...
      {/* BOTTOM NAVIGATION */}
      <div className="fixed bottom-0 left-0 right-0 max-w-sm mx-auto bg-white/95 backdrop-blur-md border-t border-slate-100 flex justify-around items-center h-20 px-2 shadow-[0_-2px_8px_rgba(0,0,0,0.04)] z-20">
...
const AccountItem = ({ title, iban, balance }) => (
  <div className="flex items-center justify-between py-5 border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors px-2 rounded-lg">
    <div className="flex items-center gap-4">
      <div className="w-11 h-11 rounded-lg bg-slate-100 flex items-center justify-center text-slate-400"><CreditCard size={22} /></div>
      <div>
